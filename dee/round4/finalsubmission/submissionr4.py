"""
Live OU-process options pricing engine — Prosperity submission.

Pipeline per tick:
    1. OU forward F = mu + (S - mu) * exp(-theta * T)  — anchors log-moneyness.
    2. WLS smile fit (Level, Skew, Convexity) with weights
           w_i = (vega_i / max(vega)) / max(spread_i, FLOOR)
       multiplied by a freshness mask (zero if quote unchanged > 400 ms).
       Sufficient stats X'WX and X'Wy are EWMA'd across ticks; coefficients
       are extracted via closed-form 3x3 inverse (no iterative solver).
    ...
    5. Trade signal-strikes whose residual r = iv_obs - iv_fit exceeds the
       half-spread vega cost: r < 0 → CHEAP, BUY; r > 0 → RICH, SELL.
"""
...
# ─── Seeds (paste from bootstrap_constants.py) ────────────────────────────
...
STALE_TS       = 400          # 400 timestamp units = 4 iterations of staleness
SPREAD_FLOOR   = 0.5
VEGA_FLOOR     = 1e-8
WING_SIGMA     = 1.5
EWMA_ALPHA     = 0.05         # EWMA on sufficient stats; half-life ≈ 14 iterations
WARMUP_NEFF    = 0.30         # need ≈ 7 effective iterations before trusting fit
...
# ─── Black-Scholes scalar (called for at most 6 strikes per tick) ────────
...
# ─── Closed-form 3x3 inverse (faster than np.linalg.solve at this size) ──
def inv3(M):
...
class Trader:
    def run(self, state: TradingState):
        ...
        # ── Per-strike read: IV, vega, spread, freshness ─────────────
        ...
        for K in STRIKES:
            ...
            fresh = 1.0 if age <= STALE_TS else 0.0       # staleness mask
        ...
        # ── Wing-clamped fitted IV (sigma uses skew at boundary) ─────
