...
# Small passive fallback for whichever S/M/L product is currently the middle leg.
MM_POS_LIMIT = 10
MM_QTY = 2
MM_EDGE = 2
MM_INV_SKEW = 2
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Trade only the two clear extremes. The middle-ranked product is not
        # forced into the basket for inventory neutrality.
        ...
        for p in TRADE_PRODUCTS:
            ...
            if cur == 0:
                ...
                if cur > MM_POS_LIMIT:
                    targets[p] = MM_POS_LIMIT
                elif cur < -MM_POS_LIMIT:
                    targets[p] = -MM_POS_LIMIT
                else:
                    targets[p] = cur
            elif abs(z) < EXIT_Z:
        ...
        middle_od = state.order_depths.get(middle)
        if middle_od is not None and not result[middle]:
            self._market_make_middle(
                middle, pos[middle], anchor, middle_od, result[middle]
            )
